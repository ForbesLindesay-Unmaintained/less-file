...
function fileLoaderCall() {
  ...
  return function () {
    ...
    token.done(function () {
      release();
      fileLoader.apply(LessParser, args);
    });
  };
}
...
function supportNpmImport(parser) {
  ...
  parser.imports.push = function (a, b, importOptions, c) {
    ...
    LessParser.fileLoader = function (file, currentFileInfo, callback, env) {
      called = true;
      var fileLoader = fileLoaderCall();
      if (importOptions.npm) {
        resolveNpmFile(file.replace(/\.less$/, ''), {
          ...
          fileLoader(newfile, currentFileInfo, callback, env);
        });
      } else {
